...
const writeFile = fs.createWriteStream(path.resolve(CSV_DIR, 'cleanedAnswersPhotos.csv'))
writeFile.write(header)

const errorRows = [];
const startTime = new Date();
console.log('Parsing answers_photos.csv...');

fs.createReadStream(path.resolve(CSV_DIR, 'answers_photos.csv'))
  .pipe(
    parse({
      delimitter: ',',
      from_line: 2,
