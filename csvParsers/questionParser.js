...
const writeFile = fs.createWriteStream(path.resolve(CSV_DIR, 'cleanedQuestions.csv'))
writeFile.write(header)

const errorRows = [];
const startTime = new Date();
console.log('Parsing questions.csv...');

fs.createReadStream(path.resolve(CSV_DIR, 'questions.csv'))
  .pipe(
    parse({
      delimitter: ',',
      from_line: 2,
